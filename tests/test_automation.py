...
from src.bridge.automation import (
    QualityMonitor, QualityAlert, QualityThreshold,
    ReportAutomation, ReportSchedule, ReportTemplate,
    NotificationSystem, NotificationChannel, AlertRule,
...
class TestQualityMonitor(unittest.TestCase):
    ...
    def test_add_monitoring_task(self):
        """모니터링 작업 추가 테스트"""
        result = self.monitor.add_monitoring_task(
            "task_1", "test_db", "users"
        )
        self.assertTrue(result)

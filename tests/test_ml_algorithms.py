...
class TestTimeSeriesAnalyzer(unittest.TestCase):
    ...
    def test_analyze_stationarity(self):
        """정상성 분석 테스트"""
        result = self.analyzer.analyze_stationarity(
            self.ts_df, 'date', 'value'
        )
    ...
    def test_decompose_time_series(self):
        """시계열 분해 테스트"""
        result = self.analyzer.decompose_time_series(
            self.ts_df, 'date', 'value'
        )
    ...
    def test_detect_trend(self):
        """트렌드 탐지 테스트"""
        result = self.analyzer.detect_trend(
            self.ts_df, 'date', 'value'
        )
    ...
    def test_detect_seasonality(self):
        """계절성 탐지 테스트"""
        result = self.analyzer.detect_seasonality(
            self.ts_df, 'date', 'value'
        )
...
class TestAnomalyDetector(unittest.TestCase):
    ...
    def test_detect_with_isolation_forest(self):
        ...
        result = self.detector.detect_with_isolation_forest(
            self.anomaly_df, contamination=0.1
        )
    ...
    def test_detect_with_one_class_svm(self):
        ...
        result = self.detector.detect_with_one_class_svm(
            self.anomaly_df, nu=0.1
        )
    ...
    def test_detect_with_zscore(self):
        """Z-Score 이상 탐지 테스트"""
        result = self.detector.detect_with_zscore(
            self.anomaly_df, threshold=3.0
        )
...
class TestClusteringAnalyzer(unittest.TestCase):
    ...
    def test_dbscan_clustering(self):
        ...
        result = self.analyzer.dbscan_clustering(
            self.cluster_df, eps=0.5, min_samples=5
        )
    ...
    def test_get_cluster_stats(self):
        """클러스터 통계 테스트"""
        clustering_result = self.analyzer.kmeans_clustering(
            self.cluster_df, n_clusters=3
        )
        
        stats = self.analyzer.get_cluster_stats(
            clustering_result, self.cluster_df
        )
...
class TestDimensionalityReducer(unittest.TestCase):
    ...
    def test_pca_reduction(self):
        """PCA 차원 축소 테스트"""
        result = self.reducer.pca_reduction(
            self.high_dim_df, n_components=3
        )
    ...
    def test_tsne_reduction(self):
        """t-SNE 차원 축소 테스트"""
        result = self.reducer.tsne_reduction(
            self.high_dim_df, n_components=2
        )
    ...
    def test_ica_reduction(self):
        """ICA 차원 축소 테스트"""
        result = self.reducer.ica_reduction(
            self.high_dim_df, n_components=3
        )
    ...
    def test_get_component_info(self):
        """주성분 정보 테스트"""
        pca_result = self.reducer.pca_reduction(
            self.high_dim_df, n_components=3
        )

...
from src.bridge.governance.contracts import ColumnSchema, DataType, QualityRule, QualityRuleType
from src.bridge.governance.metadata import Column, ColumnType, DataSource, DataSourceType, Table
...
class TestDataContract(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 테스트 컬럼 추가
        self.contract.add_column(
            ColumnSchema(
                name="id", data_type=DataType.INTEGER, nullable=False, description="Primary key"
            )
        )

        self.contract.add_column(
            ColumnSchema(
                name="name", data_type=DataType.STRING, nullable=False, description="Name field"
            )
        )

        # 품질 규칙 추가
        self.contract.add_quality_rule(
            QualityRule(
                name="not_null_name",
...
class TestContractRegistry(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.contract = DataContract(id="test_contract_1", name="Test Contract", version="1.0.0")
...
class TestMetadataCatalog(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 테스트 테이블
        self.table = Table(id="test_table_1", name="users", data_source_id="test_source_1")
...
class TestRBACManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 테스트 사용자
        self.user = User(
            id="test_user_1", username="testuser", email="test@example.com", full_name="Test User"
        )
    ...
    def test_check_permission(self):
        ...
        has_read = self.rbac.check_permission(
            "test_user_1", ResourceType.SYSTEM, None, PermissionType.READ
        )
        self.assertTrue(has_read)

        # 쓰기 권한 확인 (없어야 함)
        has_write = self.rbac.check_permission(
            "test_user_1", ResourceType.SYSTEM, None, PermissionType.WRITE
        )
...
class TestAuditLogger(unittest.TestCase):
    ...
    def test_log_authentication(self):
        ...
        result = self.audit_logger.log_authentication(
            user_id="test_user_1", username="testuser", action="login", success=True
        )
        self.assertTrue(result)
...
class TestIntegration(unittest.TestCase):
    ...
    def test_end_to_end_governance(self):
        ...
        data_source = DataSource(
            id="test_source_1", name="Test Database", type=DataSourceType.POSTGRESQL
        )
        self.metadata_catalog.register_data_source(data_source)

        # 3. 테이블 등록
        table = Table(id="test_table_1", name="users", data_source_id="test_source_1")
        self.metadata_catalog.register_table(table)

        # 4. 데이터 계약 생성
        contract = DataContract(id="test_contract_1", name="Users Contract", version="1.0.0")
        contract.add_column(ColumnSchema(name="id", data_type=DataType.INTEGER, nullable=False))

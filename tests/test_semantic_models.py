...
class TestTaskRequest:
    ...
    def test_tools_validation(self):
        ...
        # 공백 제거 - 실제로는 공백이 제거되지 않으므로 원본 값과 비교
        request = TaskRequest(intent="분석", required_tools=["  sql_executor  ", "  chart_generator  "])
        assert request.required_tools == ["  sql_executor  ", "  chart_generator  "]
...
class TestTaskStep:
    ...
    def test_valid_step(self):
        ...
        step = TaskStep(
            name="데이터 수집",
            details={"source": "postgres", "query": "SELECT * FROM customers"}
        )
...
class TestTaskStatusResponse:
    ...
    def test_status_response_validation(self):
        ...
        # 너무 긴 작업 ID
        with pytest.raises(ValidationError, match="String should have at most 100 characters"):
            TaskStatusResponse(job_id="a" * 101, state=TaskState.PENDING, ready=False, successful=False)

...
class SemanticEntity(BaseModel):
    ...
    sensitivity: SensitivityLevel = Field(
        default=SensitivityLevel.INTERNAL, description="데이터 민감도 레벨"
    )
...
class TaskRequest(BaseModel):
    ...
    required_tools: List[str] = Field(
        default_factory=list, max_length=10, description="필요한 도구 목록"
    )
    context: Dict[str, Any] = Field(
        default_factory=dict, max_length=50, description="추가 컨텍스트"
    )
...
class TaskStep(BaseModel):
    ...
    details: Dict[str, Any] = Field(
        default_factory=dict, max_length=20, description="단계 세부사항"
    )

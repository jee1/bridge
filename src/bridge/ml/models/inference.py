...
class InferenceEngine:
    ...
    def load_model(self, model_id: str) -> bool:
        ...
        try:
            ...
            if (
                model_contract.status != ModelStatus.READY
                and model_contract.status != ModelStatus.DEPLOYED
            ):
                self.logger.error(
                    f"모델이 준비되지 않았습니다: {model_id}, 상태: {model_contract.status}"
                )
                return False
    ...
    def _postprocess_predictions(
        self, predictions: np.ndarray, model_id: str, **kwargs
    ) -> Dict[str, Any]:
        """예측 결과 후처리
    ...
    def get_model_info(self, model_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "is_loaded": is_loaded,
            "performance_metrics": (
                model_contract.performance_metrics.to_dict()
                if model_contract.performance_metrics
                else None
            ),

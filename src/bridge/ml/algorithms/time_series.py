...
class TimeSeriesAnalyzer:
    ...
    def analyze_stationarity(
        self, df: UnifiedDataFrame, time_column: str, value_column: str
    ) -> Dict[str, Any]:
        """시계열의 정상성 분석
    ...
    def decompose_time_series(
        self, df: UnifiedDataFrame, time_column: str, value_column: str, model: str = "additive"
    ) -> Dict[str, Any]:
        """시계열 분해
        ...
        try:
            ...
            decomposition = seasonal_decompose(ts_data, model=model, extrapolate_trend="freq")
    ...
    def fit_arima(
        ...
        try:
            ...
            return TimeSeriesResult(
                model_type="ARIMA", predictions=np.array([]), model_metrics={"error": str(e)}
            )

    def fit_prophet(
        self, df: UnifiedDataFrame, time_column: str, value_column: str, **prophet_params
    ) -> TimeSeriesResult:
        """Prophet 모델 피팅
        ...
        try:
            ...
            return TimeSeriesResult(
                model_type="Prophet", predictions=np.array([]), model_metrics={"error": str(e)}
            )
    ...
    def forecast_arima(
        ...
        try:
            ...
            return ForecastResult(
                ...
                model_accuracy={"aic": fitted_model.aic, "bic": fitted_model.bic},
            )

        except Exception as e:
            self.logger.error(f"ARIMA 예측 중 오류 발생: {e}")
            return ForecastResult(
                forecast_values=np.array([]), forecast_dates=[], model_accuracy={"error": str(e)}
            )
    ...
    def detect_seasonality(
        ...
        try:
            ...
            return {
                ...
                and positive_fft[dominant_freq_idx] > np.mean(positive_fft),
                "dominant_frequency": dominant_freq,
                "period": period,
                "seasonal_strength": (
                    positive_fft[dominant_freq_idx] / np.mean(positive_fft)
                    if np.mean(positive_fft) > 0
                    else 0
                ),
            }

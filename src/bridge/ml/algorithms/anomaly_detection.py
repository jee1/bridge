...
class AnomalyDetector:
    ...
    def detect_with_zscore(
        self, df: UnifiedDataFrame, columns: Optional[List[str]] = None, threshold: float = 3.0
    ) -> AnomalyResult:
        """Z-Score를 사용한 이상 탐지
    ...
    def get_anomaly_stats(self, result: AnomalyResult) -> AnomalyStats:
        ...
        return AnomalyStats(
            ...
            avg_anomaly_score=(
                np.mean(result.anomaly_scores) if len(result.anomaly_scores) > 0 else 0
            ),
            max_anomaly_score=(
                np.max(result.anomaly_scores) if len(result.anomaly_scores) > 0 else 0
            ),
            min_anomaly_score=(
                np.min(result.anomaly_scores) if len(result.anomaly_scores) > 0 else 0
            ),
        )

    def compare_methods(
        self, df: UnifiedDataFrame, columns: Optional[List[str]] = None, contamination: float = 0.1
        ...
        # Isolation Forest
        results["isolation_forest"] = self.detect_with_isolation_forest(df, columns, contamination)

        # One-Class SVM
        results["one_class_svm"] = self.detect_with_one_class_svm(df, columns, contamination)
        ...
        # Z-Score
        results["z_score"] = self.detect_with_zscore(df, columns, threshold=3.0)

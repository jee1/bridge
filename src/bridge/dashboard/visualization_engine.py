...
class ChartRenderer:
    ...
    def _render_line_chart(self, data: List[Dict[str, Any]], config: ChartConfig) -> Dict[str, Any]:
        ...
        if config.x_axis and config.y_axis:
            ax.plot(
                df[config.x_axis], df[config.y_axis], color=config.color, linewidth=2, marker="o"
            )
    ...
    def _render_scatter_chart(
        ...
        if config.x_axis and config.y_axis:
            ax.scatter(df[config.x_axis], df[config.y_axis], color=config.color, alpha=0.7)
            ax.set_xlabel(config.x_axis)
            ax.set_ylabel(config.y_axis)
        else:
            # 첫 두 개의 숫자형 컬럼을 사용
            ...
            if len(numeric_cols) >= 2:
                ax.scatter(df[numeric_cols[0]], df[numeric_cols[1]], color=config.color, alpha=0.7)
...
class LayoutManager:
    ...
    def _create_grid_layout(
        self, widgets: List[Dict[str, Any]], config: LayoutConfig
    ) -> Dict[str, Any]:
        """그리드 레이아웃 생성"""
        layout = {
            "type": "grid",
...
class VisualizationEngine:
    ...
    def render_dashboard(
        self, dashboard_config: Dict[str, Any], widgets_data: Dict[str, List[Dict[str, Any]]]
    ) -> Dict[str, Any]:
        """대시보드 렌더링"""
        try:
            ...
            for widget in widgets:
                ...
                if widget_type == "chart":
                    ...
                    rendered_content = {
                        "type": "error",
                        "message": f"지원하지 않는 위젯 타입: {widget_type}",
                    }

...
class MetricCollector:
    ...
    def get_metrics_by_type(self, metric_type: MetricType, max_count: int = 100) -> List[MetricData]:
        """타입별 메트릭 조회"""
    ...
    def check_metrics(self, metrics: List[MetricData]) -> List[Alert]:
        ...
        for metric in metrics:
            for rule in self.rules.values():
                ...
                if (rule.metric_type == metric.metric_type and 
...
class RealTimeMonitor:
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connection_status": {
                client_id: status.value 
                for client_id, status in self.connection_status.items()
            },

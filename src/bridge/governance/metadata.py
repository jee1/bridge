...
class MetadataCatalog:
    ...
    def register_table(self, table: Table) -> bool:
        ...
        try:
            # 데이터 소스 존재 여부 확인
            if not table.data_source_id or table.data_source_id not in self.data_sources:
    ...
    def register_column(self, column: Column) -> bool:
        ...
        try:
            # 테이블 존재 여부 확인
            if not column.table_id or column.table_id not in self.tables:
    ...
    def search_tables(self, query: str) -> List[Table]:
        ...
        for table in self.tables.values():
            if (
                query_lower in table.name.lower()
                or query_lower in (table.description or "").lower()
            ):
    ...
    def search_columns(self, query: str) -> List[Column]:
        ...
        for column in self.columns.values():
            if (
                query_lower in column.name.lower()
                or query_lower in (column.description or "").lower()
            ):
    ...
    def get_lineage_path(self, table_id: str) -> List[DataLineage]:
        ...
        # 해당 테이블을 소스로 하는 계보들
        source_lineages = [
            lineage for lineage in self.lineage.values() if lineage.source_table_id == table_id
        ]

        # 해당 테이블을 타겟으로 하는 계보들
        target_lineages = [
            lineage for lineage in self.lineage.values() if lineage.target_table_id == table_id
        ]
    ...
    def get_full_lineage_tree(self, table_id: str) -> Dict[str, Any]:
        ...
        def build_tree(current_table_id: str, visited: Set[str]) -> Dict[str, Any]:
            ...
            result: Dict[str, Any] = {
                ...
                "source_lineages": source_entries,
                "target_lineages": target_entries,
            }
...
class DataLineageTracer:
    ...
    def _trace_upstream(self, table_id: str) -> List[Dict[str, Any]]:
        ...
        def trace_recursive(current_table_id: str, depth: int = 0):
            ...
            target_lineages = [
                lineage
                for lineage in self.catalog.lineage.values()
                if lineage.target_table_id == current_table_id
            ]

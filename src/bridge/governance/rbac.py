...
class Role:
    ...
    name: str
    description: Optional[str] = None
    permissions: List[str] = field(default_factory=list)  # Permission ID 목록
...
class User:
    ...
    roles: List[str] = field(default_factory=list)  # Role ID 목록
    is_active: bool = True
...
class RBACManager:
    ...
    def _create_default_roles(self):
        ...
        admin_permissions = [
            self._create_permission(
                "admin_all", "모든 권한", ResourceType.SYSTEM, None, PermissionType.ADMIN
            )
        ]
        ...
        read_permissions = [
            self._create_permission(
                "read_all", "모든 읽기 권한", ResourceType.SYSTEM, None, PermissionType.READ
            )
        ]
        ...
        analyst_permissions = [
            self._create_permission(
                "analytics_read", "분석 읽기", ResourceType.ANALYTICS, None, PermissionType.READ
            ),
            self._create_permission(
                "analytics_execute",
                "분석 실행",
    ...
    def check_permission(
        ...
        for perm in user_permissions:
            if (
                ...
                and (perm.resource_id is None or perm.resource_id == resource_id)
                and (
                    perm.permission_type == permission_type
                    or perm.permission_type == PermissionType.ADMIN
                )
            ):
                return True
    ...
    def create_access_token(self, user_id: str, expires_hours: int = 24) -> Optional[str]:
        ...
        access_token = AccessToken(
            token=token, user_id=user_id, expires_at=expires_at, permissions=permission_ids
        )
    ...
    def delete_role(self, role_id: str) -> bool:
        ...
        for user in self.users.values():
            if role_id in user.roles:
                self.logger.error(f"역할을 사용하는 사용자가 있습니다: {user.id}")
                return False

...
class ModelContract:
    ...
    dependencies: Dict[str, str] = field(default_factory=dict)  # 라이브러리 버전 정보
...
class DataContract:
        ...
        if "quality_rules" in data and data["quality_rules"]:
            ...
            data["quality_rules"] = [
                QualityRule(
                    name=rule["name"],
                    rule_type=(
                        QualityRuleType(rule["rule_type"])
    ...
    def validate_schema(self) -> List[str]:
        ...
        # 변환 규칙의 컬럼 존재 여부 검사
        for transform_rule in self.transformation_rules:
            if not any(col.name == transform_rule.source_column for col in self.schema):
                errors.append(
                    f"변환 규칙 '{transform_rule.name}'의 소스 컬럼 '{transform_rule.source_column}'이 스키마에 없습니다"
                )
...
class ContractValidator:
    ...
    def validate_contract(self, contract: DataContract) -> Dict[str, Any]:
        """계약 유효성 검사"""
        result: Dict[str, Any] = {"valid": True, "errors": [], "warnings": [], "schema_errors": []}
    ...
    def validate_data_against_contract(
        self, data: Dict[str, Any], contract: DataContract
    ) -> Dict[str, Any]:
        """데이터가 계약을 준수하는지 검사"""
        result: Dict[str, Any] = {"compliant": True, "violations": [], "warnings": []}
    ...
    def _validate_data_type(self, value: Any, expected_type: DataType) -> bool:
        ...
        type_mapping: Dict[DataType, Union[type, tuple[type, ...]]] = {
            DataType.STRING: str,
    ...
    def search_contracts(self, query: str) -> List[DataContract]:
        ...
        for contract in self.contracts.values():
            if (
                ...
                or query_lower in (contract.description or "").lower()
                or query_lower in (contract.source_system or "").lower()
            ):

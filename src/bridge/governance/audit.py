...
class AuditLogger:
    ...
    def _handle_high_severity_event(self, event: AuditEvent):
        ...
        # 알림 발송, 추가 로깅 등
        self.logger.warning(
            f"HIGH SEVERITY AUDIT EVENT: {event.event_type.value} - {event.details}"
        )
    ...
    def log_permission_change(
        ...
        if "role" in action.lower():
            event_type = (
                ...
                else AuditEventType.ROLE_REMOVE
            )
        else:
            event_type = (
    ...
    def get_high_severity_events(self, hours: int = 24) -> List[AuditEvent]:
        ...
        return self.get_events(
            start_time=start_time, severity=AuditSeverity.HIGH
        ) + self.get_events(start_time=start_time, severity=AuditSeverity.CRITICAL)
    ...
    def get_audit_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        summary: Dict[str, Any] = {
            ...
            "high_severity_count": len(
                [e for e in events if e.severity in [AuditSeverity.HIGH, AuditSeverity.CRITICAL]]
            ),
        }
...
class AuditTrail:
    ...
    def track_authentication(self, user_id: str, username: str, action: str, **kwargs) -> bool:
        ...
        return self.audit_logger.log_authentication(
            user_id=user_id, username=username, action=action, **kwargs
        )

    def track_permission_change(
        self, user_id: str, username: str, target_user_id: str, action: str, **kwargs
    ) -> bool:
        """권한 변경 추적"""
    ...
    def track_system_event(self, event_type: AuditEventType, **kwargs) -> bool:
        """시스템 이벤트 추적"""
        return self.audit_logger.log_system_event(event_type=event_type, **kwargs)
    ...
    def get_compliance_report(self, days: int = 30) -> Dict[str, Any]:
        ...
        data_access_events = [
            ...
            for e in events
            if e.event_type
            in [AuditEventType.DATA_READ, AuditEventType.DATA_WRITE, AuditEventType.DATA_DELETE]
        ]

        # 인증 이벤트
        auth_events = [
            e for e in events if e.event_type in [AuditEventType.LOGIN, AuditEventType.LOGOUT]
        ]

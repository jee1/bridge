...
class TaskScheduler:
        ...
        try:
            ...
            if task.function:
                ...
                result = self._run_with_timeout(
                    task.function, task.parameters, task.timeout_seconds
                )
    ...
    def get_task_status(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "successful_executions": len(
                [e for e in recent_executions if e.status == TaskStatus.COMPLETED]
            ),
            "failed_executions": len(
                [e for e in recent_executions if e.status == TaskStatus.FAILED]
            ),
        }

    def get_executions(
        self, task_id: Optional[str] = None, status: Optional[TaskStatus] = None, hours: int = 24
    ) -> List[TaskExecution]:
        """실행 기록 조회"""
        ...
        filtered_executions = [exec for exec in self.executions if exec.started_at >= cutoff_time]
    ...
    def get_scheduler_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_scheduled_task": min(
                [t.next_run for t in self.tasks.values() if t.next_run], default=None
            ),
        }

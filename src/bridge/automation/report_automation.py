...
from bridge.analytics.core import (
    UnifiedDataFrame, StatisticsAnalyzer, QualityChecker,
...
class ReportAutomation:
    ...
    def _should_run_schedule(self, schedule: ReportSchedule, now: datetime) -> bool:
        ...
        if schedule.schedule_type == ScheduleType.ONCE:
            ...
            if schedule.start_time:
                ...
                return (now >= target_time and 
                       now.weekday() == schedule.start_time.weekday())
    ...
    def execute_template(self, template_id: str, parameters: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """템플릿 즉시 실행"""
    ...
    def _fetch_data(self, template: ReportTemplate, parameters: Dict[str, Any]) -> Optional[UnifiedDataFrame]:
        """데이터 가져오기 (모의 구현)"""
        import numpy as np
        import pandas as pd
    ...
    def _generate_report(self, template: ReportTemplate, data: UnifiedDataFrame, job_id: str) -> Optional[str]:
        """리포트 생성"""
        try:
            import os
    ...
    def get_jobs(self, status: Optional[ReportStatus] = None,
                 template_id: Optional[str] = None,
                 hours: int = 24) -> List[ReportJob]:
        """작업 목록 조회"""

...
class TestQualityChecker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_data = {
            ...
            "outlier_col": ([1, 2, 3, 4, 5] * 20)[
                :100
            ],  # 이상치 포함을 위해 마지막 2개를 큰 값으로 변경
    ...
    def test_analyze_missing_values_with_columns(self):
        """특정 컬럼만 결측값 분석 테스트"""
        stats = self.checker.analyze_missing_values(self.test_df, columns=["missing_col", "value1"])
    ...
    def test_detect_outliers_with_columns(self):
        """특정 컬럼만 이상치 탐지 테스트"""
        stats = self.checker.detect_outliers(self.test_df, columns=["outlier_col", "value1"])
    ...
    def test_duplicate_data(self):
        ...
        duplicate_df = UnifiedDataFrame(
            {"id": [1, 2, 3, 1, 2, 3], "value": [10, 20, 30, 10, 20, 30]}
        )
    ...
    def test_mixed_data_types(self):
        ...
        mixed_df = UnifiedDataFrame({"mixed": ["1", "2", "3.0", "four", "5"]})  # 모든 값을 문자열로

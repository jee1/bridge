...
class SchemaMapper:
    ...
    def _create_target_schema(
        ...
        for field in source_schema:
            ...
            target_column = mapping_rules.get(field.name, field.name) if mapping_rules else field.name
    ...
    def apply_mapping(
        ...
        for column_mapping in mapping.column_mappings:
            ...
            if source_column in source_table.column_names:
                ...
                if column_mapping.transformation:
                    mapped_array = self._apply_transformation(mapped_array, column_mapping.transformation)
                
            else:
                # 컬럼이 없으면 기본값 또는 null로 채움
                if column_mapping.default_value is not None:
                    mapped_array = pa.array([column_mapping.default_value] * len(source_table), type=target_type)
                else:
                    target_type = pa.DataType.from_string(column_mapping.data_type) if column_mapping.data_type else pa.string()
        ...
        logger.info(f"스키마 매핑 적용 완료: {source_name} -> {len(mapped_table)}행, {len(mapped_table.schema)}열")
        return mapped_table
    ...
    def validate_mapping(self, source_name: str) -> Dict[str, Any]:
        ...
        """
        if source_name not in self._mappings:
            return {"is_valid": False, "error": f"소스 '{source_name}'에 대한 매핑이 존재하지 않습니다."}

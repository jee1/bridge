...
class TypeConverter:
    ...
    def _initialize_default_rules(self) -> None:
        ...
        default_rules = [
            # 정수 타입 변환
            ConversionRule(
                source_type="int8", target_type="int64", conversion_function="cast", parameters={}
            ),
            ConversionRule(
                source_type="int16", target_type="int64", conversion_function="cast", parameters={}
            ),
            ConversionRule(
                source_type="int32", target_type="int64", conversion_function="cast", parameters={}
            ),
            ConversionRule(
                source_type="uint8", target_type="int64", conversion_function="cast", parameters={}
            ),
            ConversionRule(
                source_type="uint16", target_type="int64", conversion_function="cast", parameters={}
            ),
            ConversionRule(
                source_type="uint32", target_type="int64", conversion_function="cast", parameters={}
            ),
            ConversionRule(
                source_type="uint64", target_type="int64", conversion_function="cast", parameters={}
            ),
            # 부동소수점 타입 변환
            ConversionRule(
                source_type="float32",
            ...
            ConversionRule(
                ...
                conversion_function="parse_timestamp",
                parameters={},
            ),
            ConversionRule(
                source_type="string",
            ...
            ConversionRule(
                source_type="timestamp",
                target_type="date",
                conversion_function="extract_date",
                parameters={},
            ),
            # 불린 타입 변환
            ConversionRule(
                source_type="string",
                target_type="bool",
                conversion_function="parse_bool",
                parameters={},
            ),
            ConversionRule(
                source_type="int64",
                target_type="bool",
                conversion_function="int_to_bool",
                parameters={},
            ),
            ConversionRule(
                source_type="float64",
                target_type="bool",
                conversion_function="float_to_bool",
                parameters={},
            ),
        ]
    ...
    def _is_compatible_conversion(
        self, source_type: str, target_type: str, rule: ConversionRule
    ) -> bool:
        """변환 규칙이 호환되는지 확인합니다.
    ...
    def _apply_conversion(
        self, array: pa.Array, target_type: pa.DataType, rule: ConversionRule
    ) -> pa.Array:
        """변환 규칙을 적용합니다.
    ...
    def _parse_timestamp(
        self, array: pa.Array, target_type: pa.DataType, parameters: Dict[str, Any]
    ) -> pa.Array:
        """문자열을 타임스탬프로 변환합니다.
    ...
    def validate_conversion(self, source_type: str, target_type: str) -> Dict[str, Any]:
        """변환 가능성을 검증합니다.
        ...
        for key, rule in self._conversion_rules.items():
            if self._is_compatible_conversion(source_type, target_type, rule):
                return {"is_convertible": True, "rule": rule, "method": "compatible_match"}

        return {"is_convertible": False, "rule": None, "method": "no_match"}

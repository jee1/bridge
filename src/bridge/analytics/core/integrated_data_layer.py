...
class IntegratedDataLayer:
    ...
    def __init__(
        self, chunk_size: int = 10000, memory_limit_mb: int = 1000, auto_schema_mapping: bool = True
    ):
    ...
    def integrate_data_sources(
        ...
        use_streaming = (
            enable_streaming and total_size > self.streaming_processor.memory_limit_bytes
        )

        if use_streaming:
            logger.info("스트리밍 처리 모드로 데이터 통합")
            unified_data = self._integrate_with_streaming(
                data_sources, schema_mapping, merge_strategy
            )
        else:
            logger.info("일반 처리 모드로 데이터 통합")
            unified_data = self._integrate_without_streaming(
                data_sources, schema_mapping, merge_strategy
            )
    ...
    def _create_auto_schema_mapping(
        self, data_sources: Dict[str, Union[pa.Table, pd.DataFrame, List[Dict[str, Any]]]]
    ...
    def _integrate_with_streaming(
        ...
        for source_name, data in data_sources.items():
            try:
                ...
                processed_table = self.streaming_processor.process_large_table(
                    mapped_table, lambda x, **kwargs: x  # 단순 통과 함수
                )
        ...
        return self.data_unifier.unify_data_sources(
            processed_sources, schema_mapping, merge_strategy
        )
    ...
    def apply_data_transformations(self, transformations: List[Dict[str, Any]]) -> UnifiedDataFrame:
        """데이터 변환을 적용합니다.
        ...
        for transformation in transformations:
            try:
                transformed_table = self._apply_single_transformation(
                    transformed_table, transformation
                )
            except Exception as e:
    ...
    def _apply_single_transformation(
        self, table: pa.Table, transformation: Dict[str, Any]
    ) -> pa.Table:
        """단일 변환을 적용합니다.
    ...
    def export_data(
        self, format: str = "arrow", file_path: Optional[str] = None
    ...
    def __repr__(self) -> str:
        """문자열 표현."""
        data_info = (
            f"({self._unified_data.num_rows}행, {self._unified_data.num_columns}열)"
            if self._unified_data
            else "(데이터 없음)"
        )

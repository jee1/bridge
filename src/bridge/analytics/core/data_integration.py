...
class UnifiedDataFrame:
    ...
    def _convert_to_arrow(self, data: Union[pa.Table, pd.DataFrame, Iterable[Dict[str, Any]], None]) -> pa.Table:
    ...
    def get_schema_info(self) -> Dict[str, Any]:
        ...
        schema_info = {
            ...
            "column_types": {
                name: str(field.type) for name, field in zip(
                    self.schema.names, self.schema
                )
            }
        }
        return schema_info
    ...
    def filter_rows(self, condition: str) -> UnifiedDataFrame:
        ...
        try:
            ...
            # 현재는 기본적인 비교 연산자만 지원
            filtered_table = self._table.filter(pa.compute.greater_equal(
                self._table[condition.split()[0]], 
                int(condition.split()[2])
            ))

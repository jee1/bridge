...
class StatisticalTests:
    ...
    def chi_square_test(
        self, data: UnifiedDataFrame, column1: str, column2: str, test_type: str = "independence"
    ) -> Dict[str, Any]:
        """카이제곱 검정을 수행합니다.
    ...
    def correlation_test(
        self, data: UnifiedDataFrame, column1: str, column2: str, method: str = "pearson"
    ) -> Dict[str, Any]:
        """상관관계 검정을 수행합니다.
    ...
    def ab_test_analysis(
        ...
        return {
            "test_type": test_type,
            "groups": {groups[0]: stats_a, groups[1]: stats_b},
            ...
            "recommendation": self._get_ab_test_recommendation(
                test_result.get("significant", False), practical_significance, cohens_d
            ),
        }

    def power_analysis(
        self, effect_size: float, alpha: float = 0.05, power: float = 0.8, test_type: str = "t_test"
    ) -> Dict[str, Any]:
        """검정력을 분석합니다.
        ...
        if test_type == "t_test":
            ...
            current_power = ttest_power(
                effect_size, alpha=alpha, nobs=n_required, alternative="two-sided"
            )
            ...
            return {
                ...
                "interpretation": self._interpret_power_analysis(
                    effect_size, n_required, current_power
                ),
            }
        else:
            return {"error": f"지원하지 않는 검정 유형: {test_type}"}
    ...
    def _paired_t_test(self, df: pd.DataFrame, column: str, group_column: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "difference": {"mean": (group2 - group1).mean(), "std": (group2 - group1).std()},
    ...
    def _mann_whitney_test(
        ...
        return {
            ...
            "p_value": p_value,
            "significant": p_value < self.alpha,
            "group1": {"name": groups.index[0], "median": group1.median(), "count": len(group1)},
            "group2": {"name": groups.index[1], "median": group2.median(), "count": len(group2)},
        }
    ...
    def _kruskal_wallis_test(
        ...
        for name, group in df.groupby(group_column):
            ...
            group_stats[name] = {"median": group_data.median(), "count": len(group_data)}
    ...
    def _two_way_anova(
        self, df: pd.DataFrame, column: str, group_column: str, block_column: str
    ) -> Dict[str, Any]:
        """이원분산분석을 수행합니다."""
    ...
    def _chi_square_goodness_of_fit_test(
        self, df: pd.DataFrame, column1: str, column2: str
    ) -> Dict[str, Any]:
        """카이제곱 적합도 검정을 수행합니다."""
    ...
    def _get_ab_test_recommendation(
        self, significant: bool, practical_significance: bool, cohens_d: float
    ) -> str:
        """A/B 테스트 권장사항을 제공합니다."""
    ...
    def _interpret_power_analysis(
        self, effect_size: float, n_required: float, actual_power: float
    ) -> str:
        """검정력 분석 결과를 해석합니다."""
        if actual_power >= 0.8:

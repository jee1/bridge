...
class CrossSourceJoiner:
    ...
    def register_table(self, name: str, unified_df: UnifiedDataFrame) -> None:
        ...
        try:
            ...
            logger.info(
                f"테이블 '{name}' 등록 완료: {unified_df.num_rows}행, {unified_df.num_columns}열"
            )
    ...
    def join_tables(
        self, left_table: str, right_table: str, join_condition: str, join_type: str = "inner"
    ) -> UnifiedDataFrame:
        """두 테이블을 조인합니다.
    ...
    def multi_join(
        self, tables: List[str], join_conditions: List[str], join_types: Optional[List[str]] = None
    ) -> UnifiedDataFrame:
        """다중 테이블을 조인합니다.
        ...
        try:
            ...
            for i in range(1, len(tables)):
                ...
                join_query = self._build_join_query(
                    result_table, next_table, join_condition, join_type
                )
            ...
            logger.info(
                f"다중 조인 완료: {unified_result.num_rows}행, {unified_result.num_columns}열"
            )
            return unified_result
    ...
    def _build_join_query(
        self, left_table: str, right_table: str, join_condition: str, join_type: str
    ) -> str:
        """조인 쿼리를 생성합니다.
    ...
    def execute_custom_query(self, query: str) -> UnifiedDataFrame:
        ...
        try:
            ...
            logger.info(
                f"커스텀 쿼리 완료: {unified_result.num_rows}행, {unified_result.num_columns}열"
            )
            return unified_result

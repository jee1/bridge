...
class DataUnifier:
    ...
    def unify_data_sources(
        ...
        for source_name, data in data_sources.items():
            try:
                ...
                logger.debug(
                    f"소스 '{source_name}' 변환 완료: {unified_df.num_rows}행, {unified_df.num_columns}열"
                )
        ...
        unified_schema = self._create_unified_schema(
            unified_sources, schema_mapping or {}, merge_strategy
        )

        # 3. 데이터 정규화 및 통합
        unified_data = self._merge_data_sources(
            unified_sources, unified_schema, schema_mapping or {}
        )
    ...
    def _create_unified_schema(
        ...
        # 스키마 생성
        unified_fields = [
            pa.field(column_name, column_type) for column_name, column_type in all_columns.items()
        ]
    ...
    def _unify_column_types(self, type1: pa.DataType, type2: pa.DataType) -> pa.DataType:
        ...
        type_priority = {pa.string(): 4, pa.float64(): 3, pa.int64(): 2, pa.bool_(): 1}
    ...
    def validate_unified_data(self, unified_data: UnifiedDataFrame) -> Dict[str, Any]:
        ...
        if total_columns > 0:
            avg_missing_ratio = (
                sum(info["ratio"] for info in validation_result["missing_values"].values())
                / total_columns
            )

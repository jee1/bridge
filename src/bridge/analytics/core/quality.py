...
class QualityChecker:
    ...
    def analyze_missing_values(self, df: UnifiedDataFrame, 
                             columns: Optional[List[str]] = None) -> Dict[str, MissingValueStats]:
        """결측값 분석을 수행합니다.
    ...
    def detect_outliers(self, df: UnifiedDataFrame, 
                       columns: Optional[List[str]] = None,
                       method: str = 'iqr',
                       threshold: float = 1.5) -> Dict[str, OutlierStats]:
        """이상치를 탐지합니다.
        ...
        if columns is None:
            ...
            numeric_columns = [col for col in columns if col in pandas_df.columns and 
                             pandas_df[col].dtype in [np.number, 'int64', 'float64']]
        ...
        for col in numeric_columns:
            ...
            if method == 'iqr':
                outlier_indices, outlier_values, lower_bound, upper_bound = self._detect_outliers_iqr(
                    series, threshold
                )
            elif method == 'zscore':
                outlier_indices, outlier_values, lower_bound, upper_bound = self._detect_outliers_zscore(
                    series, threshold
                )
            else:
    ...
    def _detect_outliers_iqr(self, series: pd.Series, threshold: float) -> Tuple[List[int], List[float], float, float]:
        """IQR 방법으로 이상치를 탐지합니다."""
    ...
    def _detect_outliers_zscore(self, series: pd.Series, threshold: float) -> Tuple[List[int], List[float], float, float]:
        ...
        for col in pandas_df.columns:
            ...
            # 문자열 컬럼에서 예상치 못한 패턴 확인
                ...
                if len(value_types) > 1:
                    schema_issues.append(f"컬럼 '{col}': 혼합된 데이터 타입 ({', '.join(value_types)})")
    ...
    def generate_quality_report(self, df: UnifiedDataFrame, 
        ...
        # 권장사항 및 중요 이슈 생성
        recommendations = self._generate_recommendations(missing_stats, outlier_stats, consistency_stats)
        critical_issues = self._identify_critical_issues(missing_stats, outlier_stats, consistency_stats)
    ...
    def _calculate_missing_score(self, missing_stats: Dict[str, MissingValueStats]) -> float:
        ...
        # 평균 완전성 비율을 점수로 변환
        avg_completeness = np.mean([stats.complete_ratio for stats in missing_stats.values()])
    ...
    def _calculate_outlier_score(self, outlier_stats: Dict[str, OutlierStats]) -> float:
        ...
        avg_outlier_ratio = np.mean([stats.outlier_ratio for stats in outlier_stats.values()])
        return max(0, 100 - (avg_outlier_ratio * 100))
    ...
    def _generate_recommendations(self, missing_stats: Dict[str, MissingValueStats], 
                                 outlier_stats: Dict[str, OutlierStats],
                                 consistency_stats: ConsistencyStats) -> List[str]:
        """권장사항을 생성합니다."""
        recommendations = []
        ...
        if high_missing_cols:
            recommendations.append(f"높은 결측값 비율 컬럼: {', '.join(high_missing_cols)} - 결측값 처리 방안 검토 필요")
        
        # 이상치 관련 권장사항
        high_outlier_cols = [col for col, stats in outlier_stats.items() 
                           if stats.outlier_ratio > 0.05]
        if high_outlier_cols:
            recommendations.append(f"높은 이상치 비율 컬럼: {', '.join(high_outlier_cols)} - 이상치 처리 방안 검토 필요")
    ...
    def _identify_critical_issues(self, missing_stats: Dict[str, MissingValueStats], 
                                 outlier_stats: Dict[str, OutlierStats],
                                 consistency_stats: ConsistencyStats) -> List[str]:
        """중요 이슈를 식별합니다."""
        critical_issues = []
        ...
        # 심각한 결측값 문제
        critical_missing_cols = [col for col, stats in missing_stats.items() 
                               if stats.missing_ratio > 0.5]
        if critical_missing_cols:
            critical_issues.append(f"심각한 결측값 문제: {', '.join(critical_missing_cols)} (50% 이상 결측)")
        
        # 심각한 이상치 문제
        critical_outlier_cols = [col for col, stats in outlier_stats.items() 
                               if stats.outlier_ratio > 0.2]
        if critical_outlier_cols:
            critical_issues.append(f"심각한 이상치 문제: {', '.join(critical_outlier_cols)} (20% 이상 이상치)")

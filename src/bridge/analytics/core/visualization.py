...
class ChartGenerator:
    ...
    def create_box_plot(self, 
        ...
        if config is None:
            title = f"Box Plot: {y_column}" if x_column is None else f"Box Plot: {x_column} vs {y_column}"
        ...
        if x_column is None:
            ...
            # 그룹별 박스 플롯
            data = [pandas_df[pandas_df[x_column] == group][y_column].dropna() 
...
class DashboardGenerator:
    ...
    def create_analytics_dashboard(self, 
                                 df: UnifiedDataFrame, 
                                 config: Optional[DashboardConfig] = None) -> plt.Figure:
        """분석 대시보드를 생성합니다.
        ...
        if len(numeric_columns) >= 2 and plot_idx < len(axes):
            fig_scatter = self.chart_generator.create_scatter_plot(df, numeric_columns[0], numeric_columns[1])
        ...
        for col in pandas_df.columns:
            if pandas_df[col].dtype in [np.number, 'int64', 'float64']:
                ...
                column_stats[col] = {
                    ...
                    'most_common': pandas_df[col].mode().iloc[0] if not pandas_df[col].mode().empty else None,
                    'missing_count': pandas_df[col].isnull().sum()
                }

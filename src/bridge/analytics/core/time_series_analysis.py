...
class TimeSeriesAnalysis:
    ...
    def decompose_time_series(
        ...
        try:
            ...
            return {"error": "statsmodels가 설치되지 않았습니다. pip install statsmodels를 실행하세요."}
    ...
    def _polynomial_trend_analysis(self, ts_data: pd.Series, time_index: np.ndarray) -> Dict[str, Any]:
        """다항식 트렌드 분석을 수행합니다."""
        y = ts_data.values
    ...
    def _moving_average_trend_analysis(self, ts_data: pd.Series) -> Dict[str, Any]:
        ...
        if len(moving_avg.dropna()) >= 2:
            ...
            trend_direction = "상승" if second_half > first_half else "하락" if second_half < first_half else "수평"
        else:
    ...
    def _linear_forecast(self, ts_data: pd.Series, forecast_periods: int) -> Dict[str, Any]:
        ...
        # 예측 구간 (간단한 구현)
        forecast_std = std_err * np.sqrt(1 + 1/len(ts_data) + (future_time_index - np.mean(time_index))**2 / np.sum((time_index - np.mean(time_index))**2))
    ...
    def _isolation_forest_anomaly_detection(self, ts_data: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": "scikit-learn이 설치되지 않았습니다. pip install scikit-learn을 실행하세요."}

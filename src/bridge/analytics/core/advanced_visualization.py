...
class AdvancedVisualization:
    ...
    def create_time_series_plot(
        ...
        if self.style == "plotly":
            ...
            return self._create_matplotlib_time_series(
                df, time_column, value_columns, title, config
            )
    ...
    def create_advanced_report(
        ...
        if analysis_config.get("include_visualizations", True):
            ...
            for col in numeric_columns[:5]:  # 최대 5개 컬럼
                ...
                visualizations.append({"type": "histogram", "column": col, "data": hist_info})

            # 상관관계 히트맵
            if len(numeric_columns) > 1:
                corr_plot = self._create_correlation_plot(df, "Correlation Heatmap")
                visualizations.append({"type": "correlation_heatmap", "data": corr_plot})
    ...
    def _create_plotly_dashboard(
        self, df: pd.DataFrame, chart_configs: List[Dict[str, Any]], title: str, layout: str
    ) -> Dict[str, Any]:
        """Plotly 대시보드를 생성합니다."""
        if layout == "grid":
        ...
        fig = make_subplots(
            ...
            cols=cols,
            subplot_titles=[
                config.get("title", f"Chart {i+1}") for i, config in enumerate(chart_configs)
            ],
        )
        ...
        for i, config in enumerate(chart_configs):
            ...
            if chart_type == "scatter" and x_col and y_col:
                fig.add_trace(
                    go.Scatter(
                        x=df[x_col], y=df[y_col], mode="markers", name=f"{x_col} vs {y_col}"
                    ),
                    row=row,
                    col=col,
                )
                ...
                fig.add_trace(
                    go.Bar(x=df[x_col], y=df[y_col], name=f"{x_col} vs {y_col}"), row=row, col=col
                )
    ...
    def _create_scatter_matrix(
        self, df: pd.DataFrame, title: str, config: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """산점도 행렬을 생성합니다."""
        numeric_columns = df.select_dtypes(include=[np.number]).columns.tolist()
    ...
    def _create_heatmap(
        ...
        if self.style == "plotly":
            ...
            fig, ax = plt.subplots(figsize=self.figsize)
            sns.heatmap(
                df.pivot_table(values=y_column, index=x_column, aggfunc="mean"),
                annot=True,
                fmt=".2f",
                cmap="viridis",
                ax=ax,
    ...
    def _create_correlation_plot(
        ...
        sns.heatmap(
            correlation_matrix, annot=True, cmap="coolwarm", center=0, square=True, ax=ax, fmt=".2f"
        )
        plt.title(title)
        plt.tight_layout()
    ...
    def _create_violin_plot_single(self, series: pd.Series) -> Dict[str, Any]:
        ...
        return {
            "image_base64": image_base64,
            "density_info": {"skewness": series.skew(), "kurtosis": series.kurtosis()},
        }
    ...
    def _create_density_plot(self, series: pd.Series) -> Dict[str, Any]:
        ...
        return {
            "image_base64": image_base64,
            "density_stats": {"mean": series.mean(), "std": series.std()},
        }
    ...
    def _create_ecdf_plot(self, series: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            "ecdf_stats": {
                ...
                "percentiles": {"25th": series.quantile(0.25), "75th": series.quantile(0.75)},
            },
        }
    ...
    def _create_plotly_time_series(
        ...
        for col in value_columns:
            fig.add_trace(go.Scatter(x=df[time_column], y=df[col], mode="lines+markers", name=col))

        fig.update_layout(
            title=title, xaxis_title=time_column, yaxis_title="Value", hovermode="x unified"
        )

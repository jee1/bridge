...
class DataUtils:
    ...
    @staticmethod
    def sample_data(data: Union[pa.Table, pd.DataFrame], n: int = 5) -> Union[pa.Table, pd.DataFrame]:
        """데이터 샘플을 반환합니다.
            ...
            for col_name in comparison["common_columns"]:
                ...
                if field1.type != field2.type:
                    comparison["different_columns"].append({
                        "column": col_name,
                        "type1": str(field1.type),
                        "type2": str(field2.type)
                    })
            
            # 완전히 동일한지 확인
            comparison["identical"] = (
                len(comparison["different_columns"]) == 0 and
                len(comparison["missing_in_schema1"]) == 0 and
                len(comparison["missing_in_schema2"]) == 0
            )

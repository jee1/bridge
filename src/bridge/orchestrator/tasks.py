...
from bridge.connectors.exceptions import ConnectionError, QueryExecutionError, MetadataError
...
def execute_pipeline(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"파이프라인 실행 완료: status={status}, collected={len(collected_context)}, missing={len(missing_sources)}, errors={len(errors)}")
        return result
